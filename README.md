...
KABBALAH7.py's inflow/outflow system is tinted (shifted) by one of the 4 worlds, plus the 5th default world that doesn't shift it. K7 also has Da'at (Knowledge), the "hidden" Sephirah.<br>
It correctly models that current only flows where there is a difference in potential. Just be aware that Excessive Sephiroth can lead to Deficient Paths (if they are equally excessive) and that is a feature, not a bug.
