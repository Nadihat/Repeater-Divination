...
non_reversible_runes = ['Gebo', 'Hagalaz', 'Isa', 'Jera', 'Sowilo', 'Ingwaz', 'Dagaz']
...
def draw_runes(question: str, count: int) -> List[str]:
    ...
    for i in range(count):
        ...
        while True:
            index_hash = hash_question(question, salt)
            index = index_hash % RUNE_COUNT
            if index not in used_indices:
                used_indices.add(index)
                rune_name = runes[index]
                # Decide if the rune is reversed
                if rune_name not in non_reversible_runes:
                    # Use another hash to decide if it's reversed, for determinism
                    reversed_hash = hash_question(question, salt + "-reversed")
                    if reversed_hash % 2 == 0:
                        rune_name += " (Reversed)"
                drawn.append(rune_name)
                break
    return drawn
...
def interpret_reading(question: str, rune_list: List[str], model: str) -> str:
    ...
    system_prompt = (
        ...
        "If a rune is marked as (Reversed), interpret its reversed meaning. "
