...
PLANET_POWER = {
    ...
    'Gonggong': 2, 'Quaoar': 2, 'Sedna': 3, 'Orcus': 2, 'Regulus': 6, 'Fomalhaut': 9, 'Sirius': 6, 'Spica': 5, 'Antares': 6, 'ASC': 10, 'MC': 10
}
SIGNS = [
...
# === CHART GENERATION ===
def generate_chart(seed: bytes, question: str, count: int, include_minor_bodies: bool = False) -> List[Dict[str, Any]]:
    chart = []
    used_planets = set()
    used_houses = set()
    ...
    for i in range(len(planets_to_use)):
        ...
        total_degree = hash_question(seed, question, salt + "degree") % 360
        ...
        house_index = hash_question(seed, question, salt + "house") % len(HOUSES)
...
def main():
    ...
    if reading_type == 13:
        ...
        natal_chart = generate_chart(seed, question, num_bodies, args.minor)
        transiting_chart = generate_chart(seed, f"transits for {question}", num_bodies, args.minor)
